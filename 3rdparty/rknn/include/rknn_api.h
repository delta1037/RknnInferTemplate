...
/* dummy m_init flag: could only get total_weight_size and total_internal_size by rknn_query*/
...
typedef struct _rknn_init_extend {
    ...
    int32_t      real_model_offset;                      /* real rknn model file offset, only valid when m_init context with rknn file path */
    uint32_t     real_model_size;                        /* real rknn model file size, only valid when m_init context with rknn file path */
...
/*  rknn_init
    ...
    input:
        ...
        rknn_init_extend* extend    the extend information of m_init.
    return:
        int                         error code.
*/
